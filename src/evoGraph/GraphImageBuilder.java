...
public class GraphImageBuilder {
    ...
    public GraphImageBuilder(String file){
        this.filename = file;
    }
    ...
    public BufferedImage buildImage(Graph graph){
        ...
        width  = maxX + ((double) graph.getNode(xIndex).getAttribute("width") / 2.0) + Config.borderSize;
        height = maxY + ((double) graph.getNode(yIndex).getAttribute("height") / 2.0) + Config.borderSize;
        ...
        try {
            ...
            Logger.getLogger(GraphImageBuilder.class.getName()).log(Level.SEVERE, null, ex);
        }
        return bImg;
    }
    ...
    public void drawLabel(Graphics2D g, Node node, int h){
        ...
        int y = h - (int) (xyz.y) + Config.borderSize + s + 2;
