...
public class RefinementOperator {
    ...
    protected int[] calculateIdealPoint(NodeGene roomA, NodeGene roomB) {
        double minDistance = 0;
        if (false) { // not implemented yet
            minDistance = Config.minEdgeDistance;
        }
        int[] ideal = new int[3];
        int[] pA = roomA.getXYZ();
        int[] pB = roomB.getXYZ();
        ...
        if ((pB[1] > pA[1]) && (deltaY >= deltaX)) {
            ideal[0] = pB[0];
            ideal[1] = (int) Math.max((int) (pB[1] - (roomB.getHeight() / 2) - (roomA.getHeight() / 2) - minDistance), Config.borderSize);
            ideal[2] = 0;
            ...
            ideal[0] = (int) Math.max((int) (pB[0] - (roomB.getWidth() / 2) - (roomA.getWidth() / 2) - minDistance), Config.borderSize);
            ideal[1] = pB[1];
            ideal[2] = 0;
        } // west
        else if ((pB[0] < pA[0]) && (deltaX >= deltaY)) {
