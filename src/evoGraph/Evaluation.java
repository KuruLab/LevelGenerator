...
public class Evaluation {
    ...
    public double fitness(GraphIndividual individual, boolean debug) {
        ...
        double nonLinearity = Config.useIdealNonLinearity? distanceFromIdealNonLinearity(graph) : 0;
        
        double fitness = areaIntersection + edgeIntersection + minEdgeDistance + sizePenalty + avgSP + undesiredAngleSum + nonLinearity;
        if (debug) {
            System.out.println("Size: " + individual.getNodes().size() + ", Fitness: " + fitness + 
                    ...
                    (Config.useIdealNonLinearity?   " DIN: " + nonLinearity : "") +
                    ")");
        }
    ...
    public double[] detailedFitness(GraphIndividual individual, boolean debug){
        //Decoder dungeonDecoder = new Decoder(individual);
    ...
    public double[] detailedFitness(Graph graph, boolean debug){
        ...
        double[] detailedFitness = new double[9];
        double areaIntersection = nodeAreaOverlapPenalty(graph);
        double edgeIntersection = edgeIntersectionPenalty(graph);
        ...
        double fitness =
            ...
            // Preference
            avgSP + undesiredAngleSum + nonLinearity;
    ...
    public int distanceFromIdealNonLinearity(Graph graph){
        ...
        for(Node node : targets){
            ...
            DungeonDistanceCost ddc = new DungeonDistanceCost(keyLevel);
        ...
        return Math.abs(nonlinearity - Config.idealNonLinearity);
    }
    ...
    public double minimalAngleDifference(Edge edge){
        ...
        double min = 180;
        for(int i = 0; i < Config.desiredAngles.length; i++)
            min = Math.min(min, Math.abs(Config.desiredAngles[i] - angle));
        return min;
    }
    ...
    protected double minEdgeDistancePenalty(Graph graph){
        ...
        for(Edge edge : graph.getEachEdge()){
            ...
            if(!checkAcceptableNodePosition(edge)){
                ...
                violations += Math.min(0, distance - Config.minEdgeDistance);
            }
        }
        penalty = Math.pow(violations, 2) * Config.edgeDistancePentalty;
        return penalty;
    }
    ...
    public double edgeDistance(NodeGene nodeA, NodeGene nodeB) {
    ...
    protected double nodeCountPenalty(Graph graph) {
        double penalty = 0;
        if (graph.getNodeCount() < Config.minNodeCount) {
            int delta = Config.minNodeCount - graph.getNodeCount();
            penalty = delta * delta * Config.nodeCountPenalty;
        } else {
            if (graph.getNodeCount() > Config.maxNodeCount) {
                int delta = graph.getNodeCount() - Config.maxNodeCount;
                penalty = delta * delta * Config.nodeCountPenalty;
            }
        }
        return penalty;
    ...
    protected double edgeIntersectionPenalty(Graph graph) {
        ...
        return eI * eI * Config.edgeIntersectionPenalty;
    }
    ...
    public int rectEdgeIntersection(Edge edge, Point3 p3, Point3 p4){
        ...
        double eH = (double) Config.edgeSize;
