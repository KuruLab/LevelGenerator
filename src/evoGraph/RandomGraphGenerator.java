...
public class RandomGraphGenerator {
    ...
    public GraphIndividual barabasiAlbertIndividual(String graphName){
        GraphIndividual graph = new GraphIndividual();
    ...
    public Graph barabasiAlbert(String graphName){
        ...
        gen.addSink(g);
        gen.begin();
        int numOfNodes = rng.nextInt(Config.minNodeCount) + Config.minNodeCount;
        ...
        gUtil.scalePosition(g, Config.scaleFactor, true);
    ...
    public DefaultGraph barabasiAlbert(String graphName, int size){
        ...
        util.normalizeNodesSizes(g, Config.minNodeSize, Config.maxNodeSize);
        
        util.setupStyle(g); 
        while(layout.getStabilization() < 0.95){
            layout.compute();
        }
        util.scalePosition(g, Config.scaleFactor, true);
        correctInvalidNodes(g);
    ...
    public void correctInvalidNodes(Graph graph){
        ...
        for(int i = 0; i < graph.getNodeCount(); i++){
            ...
            while(map.containsKey(point)){
                ...
                int minX = (int) Math.max(Config.borderSize, point.x - (double) node.getAttribute("width"));
                int maxX = (int) Math.min(point.x + (double) node.getAttribute("width"), width - Config.borderSize);
                point.x = rng.nextInt(maxX - minX) + minX;
                
                int minY = (int) Math.max(Config.borderSize, point.y - (double) node.getAttribute("height"));
                int maxY = (int) Math.min(point.y + (double) node.getAttribute("height"), height - Config.borderSize);
